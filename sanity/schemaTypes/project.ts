...
export const project = defineType({
  ...
  preview: {
    select: { title: "title", category: "category", location: "location", showOnWebsite: "showOnWebsite", media: "coverImage" },
    prepare: ({ title, category, location, showOnWebsite, media }) => ({
      title: title || "Proyek tanpa nama",
      subtitle: [[category, location].filter(Boolean).join(" · "), showOnWebsite === false ? "Disembunyikan" : "Tampil di website"].filter(Boolean).join(" · "),
      media,
    }),
  },
});
