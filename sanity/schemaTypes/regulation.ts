...
export const regulation = defineType({
  ...
  fields: [
    ...
    defineField({ name: "content", title: "Isi artikel", description: "Isi lengkap artikel regulasi atau pengetahuan. Gunakan paragraf pendek, heading jelas, dan gambar hanya bila benar-benar membantu.", type: "array", of: [defineArrayMember({ type: "block" }), defineArrayMember({ type: "image", options: { hotspot: true }, fields: [
      defineField({ name: "alt", title: "Alt text gambar", description: "Jelaskan isi gambar artikel secara singkat untuk aksesibilitas.", type: "string", validation: (rule) => rule.warning("Tambahkan alt text agar gambar artikel lebih aksesibel.") }),
    ], components: { input: ClientImageInput } })], fieldset: "main" }),
    defineField({ name: "coverImage", title: "Gambar cover artikel", description: "Opsional. Disiapkan untuk tampilan visual artikel Regulation bila digunakan. Gunakan gambar arsitektur landscape yang relevan dan tajam, idealnya minimal 1200px lebar. Setelah mengganti atau menghapus gambar, klik Publish.", type: "image", options: { hotspot: true }, fields: [
      defineField({ name: "alt", title: "Alt text gambar", description: "Jelaskan isi gambar cover untuk aksesibilitas dan SEO.", type: "string", validation: (rule) => rule.warning("Tambahkan alt text sebelum produksi final.") }),
    ], components: { input: ClientImageInput }, fieldset: "main" }),
